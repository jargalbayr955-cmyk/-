...
export default function LoginPage() {
  ...
  useEffect(()=>{
    let cancelled=false
    fetch('/api/customer/session',{cache:'no-store'}).then(async r=>{
      if(cancelled)return
      if(r.ok){const body=await r.json().catch(()=>({}));if(body?.user)localStorage.setItem('user',JSON.stringify(body.user));router.replace('/home')}
  ...
  return <main style={{minHeight:'100dvh',background:'radial-gradient(circle at 50% 0,#321313 0,#0a0a0f 38%,#060608 100%)',color:'white',display:'grid',placeItems:'center',padding:20}}>
    <section style={{width:'min(460px,100%)',background:'rgba(14,16,21,.92)',border:'1px solid rgba(255,255,255,.09)',borderRadius:26,padding:'28px 22px',boxShadow:'0 28px 70px rgba(0,0,0,.42)'}}>
      ...
      <p style={{margin:'0 0 24px',fontSize:13,lineHeight:1.55,color:'rgba(255,255,255,.48)'}}>SMS код шаардлагагүй. Утасны дугаар болон өөрийн PIN-ээр шууд нэвтэрнэ.</p>
