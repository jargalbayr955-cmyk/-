...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    ...
    // Дугаар бүртгэлтэй эсэх шалгах
    const { data: existing } = await supabase.from('users').select().eq('phone', fullPhone).single()
    if (existing) {
      setError('Энэ дугаар бүртгэлтэй байна. Нэвтрэх хуудас руу орно уу.')
      setLoading(false)
      return
    }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#0f0f1a' }}>
      ...
      <div className="flex-1 px-6 pt-6 pb-10">
        ...
        <button onClick={handleRegister} disabled={loading} className="w-full rounded-2xl py-4 font-medium text-sm text-white disabled:opacity-50" style={{ background: '#e8433a' }}>
