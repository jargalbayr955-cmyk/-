...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    ...
    if (!otpMode && (!/^\d{4,8}$/.test(pin) || pin !== confirmPin)) return setError('PIN 4-8 оронтой, хоёр удаа ижил байна')
    ...
    try {
      if (otpMode && !otpSent) {
        ...
        const endpoint = otpMode ? '/api/customer/otp/verify' : '/api/customer/register'
        const payload = otpMode ? { phone, token: otp } : { phone, pin }
        const res = await fetch(endpoint, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) })
        const body = await res.json().catch(()=>({}))
        if (!res.ok) setError(body.error || 'Бүртгэхэд алдаа гарлаа')
  ...
  return (
    <div style={{minHeight:'100vh', background:'#0a0a0f', display:'flex', flexDirection:'column', overflow:'hidden'}}>
      ...
      <div style={{position:'relative', height:'52vh', overflow:'hidden', background:'linear-gradient(180deg, #0d1117 0%, #1a0a0a 60%, #0a0a0f 100%)'}}>
        ...
        {[0,1,2,3,4,5].map(i => (
          <div key={i} style={{position:'absolute', bottom:'22%', left:`${i*20-5}%`, width:'12%', height:'3px', background:'rgba(255,200,0,0.15)', borderRadius:'2px'}}/>
        ))}
        {phase === 'driving' && [20,32,44,56,68].map((top, i) => (
          <div key={i} style={{position:'absolute', top:`${top}%`, left:'-20%', width:'40%', height:'1px', background:'rgba(255,255,255,0.05)', animation:`speedPass 0.4s linear ${i*0.05}s both`}}/>
        ))}
        {(phase === 'dust' || phase === 'done') && [0,1,2,3,4,5,6,7].map(i => (
          <div key={i} style={{position:'absolute', bottom:`${22+i*2}%`, left:`${40+i*4}%`, width:`${8+i*3}px`, height:`${8+i*3}px`, borderRadius:'50%', background:'rgba(180,140,80,0.3)', animation:`dustFloat${i%3} ${0.8+i*0.15}s ease-out both`, animationDelay:`${i*0.06}s`}}/>
        ))}

        <div style={{position:'absolute', bottom:'18%', left:'50%', transform:`translateX(calc(${truckX} - 50%)) skewX(${truckSkew})`, transition: phase === 'driving' ? 'transform 1s cubic-bezier(0.25,0.46,0.45,0.94)' : 'transform 0.6s cubic-bezier(0.34,1.56,0.64,1)', width:'240px'}}>
          <svg viewBox="0 0 240 90" xmlns="http://www.w3.org/2000/svg">
            ...
            <rect x="158" y="17" width="6" height="5" rx="1" fill={phase === 'done' ? '#ff4400' : '#ff8800'}/>
            <rect x="167" y="17" width="6" height="5" rx="1" fill={phase === 'done' ? '#ff8800' : '#ff4400'}/>
            <rect x="176" y="17" width="6" height="5" rx="1" fill={phase === 'done' ? '#ff4400' : '#ff8800'}/>
            <rect x="185" y="17" width="6" height="5" rx="1" fill={phase === 'done' ? '#ff8800' : '#ff4400'}/>
        ...
        <div style={{position:'absolute', bottom:'1.5rem', left:'1.5rem', right:'1.5rem', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(20px)', transition:'all 0.8s ease'}}>
      ...
      <div style={{flex:1, padding:'1.8rem 1.5rem 2.5rem', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(30px)', transition:'all 0.8s ease 0.2s'}}>
        ...
        {otpMode ? (
          ...
          <input type="password" inputMode="numeric" placeholder="PIN код (4-8 орон)" value={pin} onChange={e => setPin(e.target.value.replace(/\D/g,'').slice(0,8))}
            style={{width:'100%', boxSizing:'border-box', borderRadius:'14px', padding:'14px 16px', marginBottom:'10px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'white', fontSize:'16px', outline:'none', fontWeight:'700', letterSpacing:'3px'}}/>
          <input type="password" inputMode="numeric" placeholder="PIN дахин оруулах" value={confirmPin} onChange={e => setConfirmPin(e.target.value.replace(/\D/g,'').slice(0,8))}
        ...
        <button onClick={handleRegister} disabled={loading} style={{width:'100%', borderRadius:'16px', padding:'17px', background: loading ? 'rgba(232,67,58,0.4)' : '#e8433a', border:'none', color:'white', fontSize:'17px', fontWeight:'800', cursor: loading ? 'not-allowed' : 'pointer', letterSpacing:'0.5px', transition:'all 0.2s', boxShadow: loading ? 'none' : '0 6px 30px rgba(232,67,58,0.4)'}}>
          {loading ? 'Түр хүлээнэ үү...' : otpMode ? (otpSent ? 'Код баталгаажуулах →' : 'SMS код авах →') : 'Бүртгүүлэх →'}
        </button>
