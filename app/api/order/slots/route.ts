...
export async function POST(req:NextRequest){
 ...
 if(!(await allowRequest(`order-slots:${user.id}`,30,60_000)))return NextResponse.json({error:'Too many requests'},{status:429})
 const {order_id}=await req.json().catch(()=>({})); if(!order_id)return NextResponse.json({error:'Missing order_id'},{status:400})
 const s=getSupabaseAdmin(); const {data:order}=await s.from('orders').select('id,user_id,user_phone,status,from_lat,from_lng,driver_id,driver_name,final_price').eq('id',order_id).maybeSingle()
 ...
 if(order.status!=='pending')return NextResponse.json({order_status:order.status,selected_driver_id:order.driver_id||null,selected_driver_name:order.driver_name||null,final_price:order.final_price||null,slots:[]})
 const {data:refreshResult,error:refreshError}=await s.rpc('refresh_order_driver_slots',{p_order_id:order_id}); if(refreshError)return NextResponse.json({error:'Driver refresh failed'},{status:500})
 if(Number(refreshResult?.inserted||0)>0) await notifyOrderInvites(order_id).catch(()=>{})
 const {data:invites,error:inviteError}=await s.from('driver_invites').select('id,driver_id,rank,status,invited_at,expires_at,offered_at').eq('order_id',order_id).in('status',['active','offered']).order('rank',{ascending:true}).limit(5)
 if(inviteError)return NextResponse.json({error:'Invite lookup failed'},{status:500}); if(!invites?.length)return NextResponse.json({order_status:'pending',new_invites:Number(refreshResult?.inserted||0),slots:[]})
 const ids=invites.map(i=>i.driver_id); const [{data:drivers},{data:offers}]=await Promise.all([s.from('drivers').select('id,name,car_type,lat,lng,location_updated_at').in('id',ids),s.from('offers').select('id,driver_id,price,status,driver_lat,driver_lng').eq('order_id',order_id).in('driver_id',ids).eq('status','pending')])
