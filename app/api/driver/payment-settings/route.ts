...
import { readBankSmsConfig } from '@/lib/server/khan-bank-sms'
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ bank_name: settings.bank_name || '', bank_account: settings.bank_account || '',
    automatic_confirmation: Boolean(getPaymentWebhookSecret() && settings.bank_name && readBankSmsConfig(settings.bank_sms_config, settings.bank_account)), approval_required: false,
